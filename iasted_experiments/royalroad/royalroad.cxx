...
#define NBLOCKS 8		// this number is 2^K

const int BLOCKSIZE=4;	// block size - length of target schemata
const int GAPSIZE=3;	// gap size - number of bits between target schemata
const int MSTAR=2;	// Holland's m* - up to this many bits in low level
			// block gets reward
...
// you use that seed number.
  ...
  ga.pCrossover(0); //0.9
  ga.pMutation((float)(1/(float)nbits));
  ga.nGenerations(20000);
  ...
  while((!ga.done()))// && (stop_evolving == FALSE))
  {
    ...
    float new_mu = ga.pMutation() *2;
    if (new_mu > 0.5)
        new_mu = ((float)((float)1/(float)nbits));
