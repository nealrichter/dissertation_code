// $Header: /usr/local/cvsroot/dissertation/code/galib/iasted/expmnts/common/GASimpleGA_1p3_adaptive.cxx,v 1.4 2005/05/15 20:17:34 nealr Exp $
/* ----------------------------------------------------------------------------
  gasimple.C
  mbwall 28jul94
...
//  oldPop->initialize();
  ...
  adaptive_method = 0;
  rechenberg_success = 0;
  rechenberg_failure = 0;
  thierens_constgain_case0_success = 0;
  thierens_constgain_case1_success = 0;
  thierens_constgain_case2_success = 0;
  thierens_constgain_case3_success = 0;
  thierens_decadapt_case0_success  = 0;
  thierens_decadapt_case1_success  = 0;
  thierens_decadapt_case2_success  = 0;
  thierens_decadapt_case3_success  = 0;
}
  ...
  for(i=0; i<pop->size(); i++)
  {
    ...
    // (1 + 3) Method
    {
        ...
        for(int j = 1; j < 4; j++)
        {
            ...
            if (adaptive_method == RECHENBERG_ADAPTIVE_METHOD)
            {
                stats.nummut += (mut = sub_pop->individual( j ).mutate(pMutation()));
            }
            else if(adaptive_method == THIERENS_CONSTGAIN_ADAPTIVE_METHOD)
            {
                float mu = pMutation();
                float temp_mu = mu;
                ...
                if(j==1)
                {
                    temp_mu = temp_mu * ((float)1/THIERENS_CONSTGAIN_OMEGA);
                    stats.nummut += (mut = sub_pop->individual( j ).mutate(temp_mu));
                }
                else if(j==2)
                {
                    temp_mu = temp_mu;
                ...
                {
                    temp_mu = temp_mu * (THIERENS_CONSTGAIN_OMEGA);
                    stats.nummut += (mut = sub_pop->individual( j ).mutate(temp_mu));
                }

            }
            else if(adaptive_method == THIERENS_DECLINING_ADAPTIVE_METHOD)
            {
                ...
                if(j==1)
                {
                    temp_mu = temp_mu * (THIERENS_DECADAPT_OMEGA);
                    stats.nummut += (mut = sub_pop->individual( j ).mutate(temp_mu));
                }
                else if(j==2)
                {
            ...
            {
                cerr << "No adaptive method specififed, exiting\n\n";
                exit(1);
            }
        ...
        if (adaptive_method == RECHENBERG_ADAPTIVE_METHOD)
        {
            pop->individual( i ).copy(sub_pop->individual( max_index )); //Pick max
        }
        else if(adaptive_method == THIERENS_CONSTGAIN_ADAPTIVE_METHOD)
        {
            if(max_index==0)
                thierens_constgain_case0_success++;
            else if(max_index==1)
                thierens_constgain_case1_success++;
            else if(max_index==2)
                thierens_constgain_case2_success++;
            else if(max_index==3)
                thierens_constgain_case3_success++;
        ...
        {
            if(max_index==0)
                thierens_decadapt_case0_success++;
            else if(max_index==1)
                thierens_decadapt_case1_success++;
            else if(max_index==2)
                thierens_decadapt_case2_success++;
            else if(max_index==3)
                thierens_decadapt_case3_success++;
...
int GASimpleGA_1p3::adaptiveMethod(int new_adap_method)
{
    adaptive_method = new_adap_method;
    return(adaptive_method);
}

int GASimpleGA_1p3::adaptiveMethod()
{
    return(adaptive_method);
}

float GASimpleGA_1p3::updateMutation(float low, float high)
{
    float new_mu =  pMutation();

    if (adaptive_method == RECHENBERG_ADAPTIVE_METHOD)
    {
        float success = (float) rechenberg_success / ((float)rechenberg_success +  rechenberg_failure);

        if (success > 0.20)
            new_mu = new_mu * RECHENBERG_ALPHA;
        else if (success < 0.20)
            new_mu = new_mu / RECHENBERG_ALPHA;

        //cerr << "RECHENBERG_ADAPTIVE_METHOD success = " << success << endl;

        rechenberg_success = rechenberg_failure = 0;

    }
    else if(adaptive_method == THIERENS_CONSTGAIN_ADAPTIVE_METHOD)
    {

        if((thierens_constgain_case0_success > thierens_constgain_case1_success) &&
            (thierens_constgain_case0_success > thierens_constgain_case2_success) &&
            (thierens_constgain_case0_success > thierens_constgain_case3_success))
        {
            new_mu =  pMutation(); //no-op
        }
        else if((thierens_constgain_case1_success > thierens_constgain_case0_success) &&
            (thierens_constgain_case1_success > thierens_constgain_case2_success) &&
            (thierens_constgain_case1_success > thierens_constgain_case3_success))
        {
            new_mu =  pMutation() * ((float)1/THIERENS_CONSTGAIN_LAMBDA); //decrease
        }
        else if((thierens_constgain_case2_success > thierens_constgain_case0_success) &&
            (thierens_constgain_case2_success > thierens_constgain_case1_success) &&
            (thierens_constgain_case2_success > thierens_constgain_case3_success))
        {
            new_mu =  pMutation(); //no-op
        }
        else if((thierens_constgain_case3_success > thierens_constgain_case0_success) &&
            (thierens_constgain_case3_success > thierens_constgain_case1_success) &&
            (thierens_constgain_case3_success > thierens_constgain_case2_success))
        {
            new_mu =  pMutation() * (THIERENS_CONSTGAIN_LAMBDA); //increase
        }
    ...
    {
        if((thierens_decadapt_case0_success > thierens_decadapt_case1_success) &&
            (thierens_decadapt_case0_success > thierens_decadapt_case2_success) &&
            (thierens_decadapt_case0_success > thierens_decadapt_case3_success))
        {
            new_mu =  pMutation() * THIERENS_DECADAPT_GAMMA; //decrease
        }
        else if((thierens_decadapt_case1_success > thierens_decadapt_case0_success) &&
            (thierens_decadapt_case1_success > thierens_decadapt_case2_success) &&
            (thierens_decadapt_case1_success > thierens_decadapt_case3_success))
        {
            new_mu =  pMutation() * (THIERENS_CONSTGAIN_LAMBDA); //increase
        }
        else if((thierens_decadapt_case2_success > thierens_decadapt_case0_success) &&
            (thierens_decadapt_case2_success > thierens_decadapt_case1_success) &&
            (thierens_decadapt_case2_success > thierens_decadapt_case3_success))
        {
            new_mu =  pMutation(); //no-op
        }
        else if((thierens_decadapt_case3_success > thierens_decadapt_case0_success) &&
            (thierens_decadapt_case3_success > thierens_decadapt_case1_success) &&
            (thierens_decadapt_case3_success > thierens_decadapt_case2_success))
        {
            new_mu =  pMutation() * (THIERENS_CONSTGAIN_LAMBDA); //increase
        }
    ...
    //cap mut growth & shrink
    if(new_mu > high)
        new_mu = high;
    else if(new_mu < low)
        new_mu = low;

    return(pMutation(new_mu));
